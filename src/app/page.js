...
// --- INITIAL MOCK DATA (Enriched with Tech Stack, Source, and Crisis Events) ---
// Updated: EmailBisous -> EmailBison
...
// --- SIMULATED DATA FOR GOLDEN WINDOW (BigQuery Aggregation Mock) ---
// 1 = Low yield (light), 4 = High yield (dark/bright)
const GOLDEN_WINDOW_DATA = [
 { day: 'Mon', hours: [1, 2, 2, 3, 2, 1, 1, 0] },
 { day: 'Tue', hours: [2, 3, 4, 4, 4, 3, 2, 1] }, // The "Golden" Day
...
const Card = ({ title, subtitle, children, className = "", actionItem }) => (
 <div className={`bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-sm flex flex-col ${className}`}>
...
const SidebarItem = ({ icon: Icon, label, active, isOpen, notification }) => (
 <div className={`flex items-center justify-between px-4 py-3 rounded-lg cursor-pointer transition-all ${
   active
...
export default function AgencyDashboard() {
 ...
 useEffect(() => {
   ...
   // Default to Stark Industries to show the crisis immediately
 ...
 // 1. Inbox Health (The "Ops" Context - Crisis Detection)
 ...
 // 2. Tech Stack Intelligence (The "Clay Enrichment" Flex)
 const techStackData = useMemo(() => {
   ...
   return Object.values(stacks)
     ...
     .sort((a, b) => b.rate - a.rate); // Sort by highest win rate
 }, [filteredData]);


 // 3. Goal Tracking (Actual vs Planned)
 const goalData = useMemo(() => {
   const timeline = {};
   filteredData.forEach(row => {
     if (!timeline[row.Date]) timeline[row.Date] = { date: row.Date, actual: 0, goal: 0 };
     timeline[row.Date].actual += row.Meetings_Booked;
     timeline[row.Date].goal += row.Daily_Meeting_Goal; // Summing goal if multiple rows per day
   });
   return Object.values(timeline).sort((a, b) => new Date(a.date) - new Date(b.date));
 }, [filteredData]);


 // 4. Source Breakdown (Unified Data Flex)
 ...
 const hasHighBounceRate = useMemo(() => {
   ...
   return (totalBounces / totalSent) > 0.05; // 5% Threshold
 }, [filteredData]);
 ...
 return (
   <div className="flex h-screen bg-slate-900 text-slate-100 font-sans overflow-hidden">
     ...
     {/* SIDEBAR */}
     <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-slate-950 border-r border-slate-800 transition-all duration-300 flex flex-col z-20`}>
       <div className="h-16 flex items-center px-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            {isSidebarOpen && <span className="font-bold text-lg tracking-tight">GTMA <span className="text-indigo-500">OS</span></span>}
          </div>
       </div>


       <nav className="flex-1 py-6 px-3 space-y-2">
         <SidebarItem icon={LayoutDashboard} label="Command Center" active isOpen={isSidebarOpen} />
         <SidebarItem icon={Users} label="Prospects" isOpen={isSidebarOpen} />
         <SidebarItem icon={Briefcase} label="Campaigns" isOpen={isSidebarOpen} />
        
         <div className="pt-4 pb-2">
           {isSidebarOpen && <p className="px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Intelligence</p>}
           <SidebarItem icon={ShieldCheck} label="Enrichment Validator" isOpen={isSidebarOpen} />
           <SidebarItem icon={Layers} label="Tech Stack Analysis" isOpen={isSidebarOpen} notification="New" />
           <SidebarItem icon={Clock} label="Golden Window" isOpen={isSidebarOpen} notification="AI" />
         </div>


         <div className="pt-4 pb-2">
           {isSidebarOpen && <p className="px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">RevOps Monitor</p>}
           <SidebarItem icon={MailWarning} label="Inbox Health" isOpen={isSidebarOpen} notification={hasHighBounceRate ? "!" : null} />
           <SidebarItem icon={Database} label="Data Warehouse" isOpen={isSidebarOpen} />
         </div>
       </nav>
       ...
       <div className="p-4 border-t border-slate-800">
         <div className="flex items-center gap-3 cursor-pointer hover:bg-slate-900 p-2 rounded-lg transition-colors">
           <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 flex items-center justify-center text-xs font-bold">
             JD
           </div>
           {isSidebarOpen && (
     ...
     <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
       ...
       <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-8 z-10">
         <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-semibold text-white tracking-tight flex items-center gap-2">
              ...
              {hasHighBounceRate && <span className="bg-rose-500/10 text-rose-400 text-xs px-2 py-0.5 rounded border border-rose-500/20">Critical Alert</span>}
            </h2>
         </div>
         ...
         <div className="flex items-center gap-4">
           ...
           <div className="hidden md:flex items-center px-3 py-1.5 bg-slate-800/50 rounded-lg border border-slate-800 text-slate-400 text-sm">
              <span>Oct 2025</span>
       ...
       <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
           ...
           <Card title="Revenue Forecast" subtitle="Meetings Booked vs. Daily Goals" className="lg:col-span-8 h-[400px]">
           ...
           <Card title="Traffic Source" subtitle="Volume by Tool" className="lg:col-span-4 h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={sourceData}
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
           ...
           <Card
             ...
             className="lg:col-span-8 h-[400px]"
             actionItem={<div className="text-xs px-2 py-1 bg-emerald-500/10 text-emerald-400 rounded border border-emerald-500/20">AI Recommended: Tue 11AM - 2PM</div>}
           >
              <div className="flex-1 flex flex-col justify-center">
                <div className="grid grid-cols-9 gap-2">
                   {/* Header Row (Hours) */}
                   <div className="text-xs font-bold text-slate-500 text-right pr-2">Day/Time</div>
                   {HOURS_LABEL.map(h => (
                     <div key={h} className="text-xs font-medium text-slate-500 text-center">{h}</div>
                   ))}
             ...
             actionItem={
               hasHighBounceRate && (
                 <div className="flex items-center gap-2 bg-rose-500/10 px-3 py-1 rounded-full border border-rose-500/20">
                   ...
                   <span className="text-xs font-bold text-rose-400">CRITICAL: OUTLOOK SPIKE</span>
                 </div>
               )
             }
           >
         ...
         <div className="mt-8 flex items-center justify-center space-x-2 text-slate-600 text-xs">
           ...
           <span>Data Warehouse: BigQuery (Connected via Unified Schema)</span>
